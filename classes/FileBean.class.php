...
class FileBean extends DatabaseBean
{
     ...
     * into the database. If the filename of the file corresponds to an already
    ...
    /**
     * Shuffle file positions according to the parameter.
     */
    function shuffleFiles($fileList)
    {
        foreach ($fileList as $pos => $id)
        {
            $this->dbQuery('UPDATE file SET position=' . $pos . ' WHERE id=' . $id);
        }
    }
        ...
        if (UserBean::isRoleAtLeast(SessionDataBean::getUserRole(), USR_LECTURER))
        {
            ...
            foreach ($rs as $val)
            {
                ...
                if (is_file($fn))
                {
                    ...
                    $stb->id = $val['uid'];
                    $stb->dbQuerySingle();
                    $u8name = $stb->getFirstName() . " " . $stb->getSurname();
