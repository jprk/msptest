...
class FormSolutionsBean extends DatabaseBean
{
  ...
  function _getFullList ( $where = '' )
    ...
    $excersiseBean = new ExcersiseBean ( 0, $this->_smarty, "x", "x");
		...
		$lectureBean = new LectureBean ( 1, $this->_smarty, "x", "x" );
		...
		$excersiseBinding = $studexcBean->getExcersiseBinding ( $excersiseList );
	  ...
	  $studentBean = new StudentBean (0, $this->_smarty, "x", "x");
		$studentList = $studentBean->assignStudentListWithExcersises ( count ( $excersiseList ), $excersiseBinding );
