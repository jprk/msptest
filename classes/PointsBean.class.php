...
class PointsBean extends DatabaseBean
{
        ...
        if (isset ($subtaskList))
        {
            foreach ($subtaskList as $key => $val)
            {
                ...
                /* TODO: Hack. Better to setup up PTS_NOT_CLASSIFIED for the student
                      ...
                      Not classified means that the record is completely empty. */
                ...
                $subtaskList[$key]['comment'] = $tsc;
                /* And store them also the the SESSION so that they can be displayed in a popup window
                   with minimum additional work. */
                $_SESSION['comment'][$subtaskId] = $tsc;
            }
        }
    ...
    function doEdit()
    {
        ...
        /* Prepare the listing based on the type of listing that has
        ...
        switch ($this->type)
        {
            case 'exc':
                ...
                $this->_smarty->assign('islocked', 1);
                ...
                switch ($res)
                {
                    ...
                    case MUTEX_LOCK_STOLEN_OK;
                        /* Stealing a stale lock is perfectly okay. On the other
            ...
            case 'lec':
                /* Lecture-wide view works with AJAX form editing, meaning that points
                   are being saved student by student, subtask by subtask. There is no
                   need to lock the whole table here. */
    ...
    /* -------------------------------------------------------------------
       HANDLER: SAVE
        ...
        TODO: Think about a meaningful set of return codes for AJAX.
       ------------------------------------------------------------------- */
    function doSave()
    {
        /* Assign POST variables to internal variables of this class and
        ...
        /* Update all the records.
           TODO: Wrap it with mutexLock() and mutexUnlock for type=='lec'.
           When this happens, the unlocking part may be moved out of the
           next if branch as it will apply to both. */
        $this->dbReplace();
