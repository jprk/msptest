...
převeďte prosím na desetinná čísla. Jako oddělovač desetinné části lze použít jak
čárku, tak i tečku.
</p>
<p>
Odpovídat můžete pouze jednou.
</p>
<form name="solutionform" action="?act=verify,formsolution,{$subtask.id}" method="post" onsubmit="">
...
<h3>Úloha {$subtask.ttitle}-{$assignment.assignmnt_id|string_format:"%05d"}{$parts[pId].part}</h3>
  ...
  (&nbsp;{$varList.$temp}&nbsp;-&nbsp;(&nbsp;<input type="text" name="a[{$parts[pId].part}]" size="3" onblur="validnumber(this);">+<input type="text" name="b[{$parts[pId].part}]" size="3" onblur="validnumber(this);">i&nbsp;))
  (&nbsp;{$varList.$temp}&nbsp;-&nbsp;(&nbsp;<input type="text" name="c[{$parts[pId].part}]" size="3" onblur="validnumber(this);">+<input type="text" name="d[{$parts[pId].part}]" size="3" onblur="validnumber(this);">i&nbsp;))
  (&nbsp;{$varList.$temp}&nbsp;-&nbsp;(&nbsp;<input type="text" name="e[{$parts[pId].part}]" size="3" onblur="validnumber(this);">+<input type="text" name="f[{$parts[pId].part}]" size="3" onblur="validnumber(this);">i&nbsp;)
  </div>
  </div>
  </p>
  <p>
  Systém je
  <input type="radio" name="g[{$parts[pId].part}]" value="2" required="required">nestabilní
  <input type="radio" name="g[{$parts[pId].part}]" value="1">na mezi stability
  <input type="radio" name="g[{$parts[pId].part}]" value="0">stabilní.
  </p>
{/section}
<p>
<input type="submit" value="Zkontrolovat řešení před odevzdáním">
