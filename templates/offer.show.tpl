    ...
    {section name=articlePos loop=$articleList}
        <tr>
            {if $articleList[articlePos].imgId}
                <td valign="top"><img src="ctrl.php?act=show,files,{$articleList[articlePos].imgId}"
                                      alt="{$articleList[articlePos].imgAlt}"></td>
            {else}
                <td valign="top"><img src="images/e75.gif" alt="[placeholder]"></td>
            {/if}
            <td valign="top">
                <p class="otitle">{if $isAdmin || $isLecturer}<span class="editimg"
                ...
                {if $articleList[articlePos].text}
                <div class="otext">{$articleList[articlePos].text}</div>{/if}
...
<input type="submit" value="Objedn�vka">
</form>
{/if}
</td>
</tr>
