...
<table class="admintable" border="0" cellpadding="2" cellspacing="1">
    ...
    <tr class="rowB" style="vertical-align: baseline;">
        ...
        <td>ANO</td>
            {elseif $replacement.failed}
        <td>NE TEST</td>
            {elseif $replacement.dateto and $replacement.dateto < $replacement.fromtime}
        <td>ZRU&nbsp;{$replacement.dateto|date_format:"%d.%m.%Y %H:%M"}</td>
            {elseif $replacement.dateto}
        <td>OML&nbsp;{$replacement.dateto|date_format:"%d.%m.%Y %H:%M"}</td>
