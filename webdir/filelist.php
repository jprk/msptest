...
if ( UserBean::isRoleAtLeast ( SessionDataBean::getUserRole(),USR_LECTURER ))
{
    try
    {
        /* Create an instance of database interface to the `file` table. */
        $fileBean = new FileBean( NULL, $smarty, NULL, NULL );
        /* If submitted correctly, $_POST array will contain a single key `files` holding an
           array of file ids after the user moved a file item to another position. */
        $fileBean->shuffleFiles( $_POST['file'] );
        $status = 0;
    }
    catch ( Exception $e )
    {
